...
for h in xrange(start_index, end_index + 1):
    ...
    for i in xrange(0, player_count):
        ...
        move = move_cleaned[i]
        move_up_xpath = "//table[@class='mega-table']/tbody/tr[" + str(
            i + 1) + "]/td[@class='move-cell']/div[@class='move-up']"
        move_up_parsed = xpath_parse(week_tree, move_up_xpath)
        move_down_xpath = "//table[@class='mega-table']/tbody/tr[" + str(
            i + 1) + "]/td[@class='move-cell']/div[@class='move-down']"
        ...
        data = [week_title, week_year, week_month, week_day, rank_text, rank_number, move, move_direction, age, points,
                tourneys, player_url, player_slug, player_id]
